...
def construct_request(
    ...
    if st.session_state.discussion:
        request += (
            f" The discussion so far has been {st.session_state.discussion[-50000:]}."
        )
    if skill_results:
...
def display_agents():
    if "agents" in st.session_state and st.session_state.agents:
        ...
        st.sidebar.warning(
            f"No agents have yet been created. Please enter a new request."
        )
        st.sidebar.warning(
            f"NOTE: GPT models can only be used locally, not in the online demo."
        )
        st.sidebar.warning(
            f"ALSO: If no agents are created, do a hard reset (CTL-F5) and try switching models. LLM results can be unpredictable."
        )
        st.sidebar.warning(
            f"SOURCE:  https://github.com/jgravelle/AutoGroq\n\r\n\r https://j.gravelle.us"
        )
...
def display_agent_buttons(agents):
    for index, agent in enumerate(agents):
        ...
        with col2:
            ...
            st.button(
                agent_name, key=f"agent_{index}", on_click=agent_button_callback(index)
            )
...
def display_agent_edit_form(agent, edit_index):
    with st.expander(
        ...
        with col2:
            ...
            if st.session_state.get(f"delete_confirmed_{edit_index}", False):
                if container.button(
                    "Confirm Deletion", key=f"confirm_delete_{edit_index}"
                ):
                    st.session_state.agents.pop(edit_index)
                    st.session_state["show_edit"] = False
...
def download_agent_file(expert_name):
    ...
    formatted_expert_name = formatted_expert_name.lower().replace(
        " ", "_"
