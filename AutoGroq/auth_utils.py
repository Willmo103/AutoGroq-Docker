        ...
        if api_key is None:
            if api_key_env_var not in st.session_state:
                api_key = st.text_input(
                    f"Enter the {LLM_PROVIDER.upper()} API Key:",
                    type="password",
                ...
                if api_key:
                    ...
                    st.warning(
                        f"Please enter the {LLM_PROVIDER.upper()} API Key to use the app."
                    )
            else:
        ...
        if api_url is None:
            if api_url_env_var not in st.session_state:
                api_url = st.text_input(
                    f"Enter the {LLM_PROVIDER.upper()} API URL:",
                    type="password",
                ...
                if api_url:
                    ...
                    st.warning(
                        f"Please enter the {LLM_PROVIDER.upper()} API URL to use the app."
                    )
            else:
